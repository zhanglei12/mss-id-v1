...
function plan($plan, $why = '') {
    ...
    $_PHP_Test['planned'] = true;

    switch ($plan) {
        case 'no_plan':
            $_PHP_Test['planned'] = false;
            break;
        case 'skip_all';
            printf("1..0%s\n", $why ? " # Skip $why" : '');
            exit;
...
function is($have, $want, $desc = '') {
    $pass = $have == $want;
    return _proclaim($pass, $desc, /* todo */
        false, $have, $want);
}
...
function _proclaim($cond, # bool
    ...
    if ($todo) {
        $todo_idx = count($_PHP_Test['todo']) - 1;
        $directive .= ' # TODO ' . $_PHP_Test['todo'][$todo_idx];
    }

    printf("%s %d %s%s\n", $ok, $_PHP_Test['run'], $desc, $directive);
...
function _test_ends() {
    ...
    if (count($_PHP_Test['todo']) != 0) {
        $todos = join("', '", $_PHP_Test['todo']);
        die("Missing todo_end() for '$todos'");
    }

    if (!$_PHP_Test['planned']) {
        printf("1..%d\n", $_PHP_Test['run']);
    }
}
