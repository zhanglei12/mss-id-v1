...
class _Search
{
	...
	function search($search){
		...
		//Search-判断订单来源
		$csOrderType = $search['csOrder'];
		if($csOrderType != '' && $csOrderType != 0){
			if($csOrderType == 'cs'){
				$searchSql .= " AND a.cs_order_type IN ( 2,3 )";
				$csOrderType = 2;
			}else{
				$searchSql .= " AND a.cs_order_type = ".$csOrderType;
			}
			$data['csOrderType'] = $csOrderType;
		}
		// Search-区域
		...
		if($search['search']) {
			...
			if($search['partner_order_id'] != '') {
				...
				$searchSql .= " AND a.partner_order_id LIKE '%".$partner_order_id."%' ";
			}
			// Search-收货人用户名
			if($search['consignee'] != '') {
				$consignee = $search['consignee'];
				$data['consignee'] = $consignee;
				$searchSql .= " AND (b.consignee LIKE '%".$consignee."%' OR a.buyer_name LIKE '%".$consignee."%') ";
			}
			// Search-收货人电话 
			if($search['phone_mob'] != '') {
				$phone_mob = $search['phone_mob'];
				$data['phone_mob'] = $phone_mob;
				$searchSql .= " AND (b.phone_mob LIKE '%".$phone_mob."%' OR j.phone_mob LIKE '%".$phone_mob."%') ";
			}
			// Search-店铺名称 
			...
			if($search['payment_name'] != '') {
				...
				$searchSql .= " AND a.payment_name LIKE '%".$payment_name."%' ";
			}
			// ORDER BY
			if($search['orderBy'] != '') {
